        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">师资力量收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shizililiangUuidNumber" id="shizililiangUuidNumber" placeholder="老师工号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shizililiangName" id="shizililiangName" placeholder="老师名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                         
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">所教专业</label>
                              <div class="layui-input-inline">
                                  <select name="shizililiangTypes" id="shizililiangTypes" lay-filter="shizililiangTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in shizililiangTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <div class="portfolio-agile">
                            <div class="main">
                                <div v-for="(item,index) in dataList" v-bind:key="index" class="view view-seventh" style="width: 23%;height: 300px">
                                    <i @click="deleteShizililiangCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <a @click="jumpCheck('../shizililiang/detail.jsp?id='+item.shizililiangId , item.shizililiangDelete == null?'':item.shizililiangDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="b-link-stripe b-animate-go thickbox">
                                        <img :src="item.shizililiangPhoto?item.shizililiangPhoto.split(',')[0]:''"  style="height: 350px;width:100%;" />
                                        <div class="mask">
                                            <h4>
                                                <b>{{item.shizililiangName}}</b>
                                                <br>
                                                <b v-if="item.shizililiangNewMoney">{{item.shizililiangNewMoney}} RMB</b>
                                                <b v-else>{{item.shizililiangValue}}</b>
                                            </h4>
                                        </div>
                                    </a>
                                </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            shizililiangCollectionTypesList : [],
        // 级联表字典表数据
            shizililiangTypesList : [],
            shangxiaTypesList : [],
            shizililiangCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteShizililiangCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('shizililiangCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function shizililiangCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shizililiang_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shizililiangCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function shizililiangTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shizililiang_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shizililiangTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#shizililiangUuidNumber').val() != null && jquery('#shizililiangUuidNumber').val() != "" && jquery('#shizililiangUuidNumber').val() != "null" ) {
                param['shizililiangUuidNumber'] = jquery('#shizililiangUuidNumber').val();
            }
                     
            if (jquery('#shizililiangName').val() != null && jquery('#shizililiangName').val() != "" && jquery('#shizililiangName').val() != "null" ) {
                param['shizililiangName'] = jquery('#shizililiangName').val();
            }
                                         
            var shizililiangTypes = $("#shizililiangTypes").find("option:selected").val();
            if (shizililiangTypes != null && shizililiangTypes != "" && shizililiangTypes != "null") {
                            param['shizililiangTypes'] = shizililiangTypes;
                        }
                                                                                                                        
            // 当前表
                                                
            //收藏只能查类型为1的
            param['shizililiangCollectionTypes'] = 1;

            // 获取列表数据
            http.request('shizililiangCollection/page', 'get', param, function(res) {
