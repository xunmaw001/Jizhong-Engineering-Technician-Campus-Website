...
<body>
        ...
        <section class="blog-details-section pt-120 pb-120">
            <div class="data-detail-breadcrumb" style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.shizililiangCollectionName}}</cite></a>
                </span>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.shizililiangCollectionPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.shizililiangCollectionTypes">类型：
                                {{detail.shizililiangCollectionValue}}
                            </li>
                        </ul>
                        ...
                        <!--<div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                            <div class="num-picker">
                                <button @click="shizililiangCollectionYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                    立即预约
                                </button>
                            </div>
                        </div>-->
        ...
        <div class="col-lg-4" v-if="shizililiangCollectionRecommendList">
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in shizililiangCollectionRecommendList" v-bind:key="index" @click="jump('../shizililiangCollection/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.shizililiangCollectionPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.shizililiangCollectionName}}</a>
                                <h6 v-if="item.shizililiangCollectionNewMoney" class="post-title"><a style="color: red">{{item.shizililiangCollectionNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            //系统推荐
            shizililiangCollectionRecommendList: [],
            dataList: [],
            // 当前详情页表
            detailTable: 'shizililiangCollection',
        },
        //  清除定时器
        destroyed: function () {
            // 不知道具体作用
        ...
        methods: {
            ...
            //预约
            ,shizililiangCollectionYuyue(){
                let _this = this;
                // localStorage.setItem('shizililiangCollectionId', _this.detail.id);
                // _this.jump("../shizililiangCollectionOrder/add.jsp");

                let data={
                    shizililiangCollectionId:_this.detail.id,
                    xueshengId:localStorage.getItem("userid"),
                    shizililiangCollectionOrderYesnoTypes:1,
                }
                // 提交数据
                layui.http.requestJson('shizililiangCollectionOrder/add', 'post', data, function (res) {
                    if(res.code == 0) {
                        layui.layer.msg('预约成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            _this.jump("../shizililiangCollectionOrder/list.jsp");
                        });
                    }else{
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'rate'], function () {
        ...
        http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
            // 详情信息
            vue.detail = res.data;
            vue.title = vue.detail.shizililiangCollectionName;
           // 轮播图片
            vue.swiperList = vue.detail.shizililiangCollectionPhoto ? vue.detail.shizililiangCollectionPhoto.split(",") : [];
            var swiperItemHtml = '';
        ...
        http.request(`shizililiangCollection/list`, 'get', {
            page: 1,
            limit: 5,
            shizililiangCollectionTypes: vue.detail.shizililiangCollectionTypes,




        }, function (res) {
            vue.shizililiangCollectionRecommendList = res.data.list;
        });
