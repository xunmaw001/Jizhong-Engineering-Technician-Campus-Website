        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">校园风光收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">风光编号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="xiaoyuanfengguangUuidNumber" id="xiaoyuanfengguangUuidNumber" placeholder="风光编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">校园风光名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="xiaoyuanfengguangName" id="xiaoyuanfengguangName" placeholder="校园风光名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                         
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">校园风光类型</label>
                              <div class="layui-input-inline">
                                  <select name="xiaoyuanfengguangTypes" id="xiaoyuanfengguangTypes" lay-filter="xiaoyuanfengguangTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in xiaoyuanfengguangTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <div class="portfolio-agile">
                            <div class="main">
                                <div v-for="(item,index) in dataList" v-bind:key="index" class="view view-seventh" style="width: 23%;height: 300px">
                                    <i @click="deleteXiaoyuanfengguangCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <a @click="jumpCheck('../xiaoyuanfengguang/detail.jsp?id='+item.xiaoyuanfengguangId , item.xiaoyuanfengguangDelete == null?'':item.xiaoyuanfengguangDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="b-link-stripe b-animate-go thickbox">
                                        <img :src="item.xiaoyuanfengguangPhoto?item.xiaoyuanfengguangPhoto.split(',')[0]:''"  style="height: 350px;width:100%;" />
                                        <div class="mask">
                                            <h4>
                                                <b>{{item.xiaoyuanfengguangName}}</b>
                                                <br>
                                                <b v-if="item.xiaoyuanfengguangNewMoney">{{item.xiaoyuanfengguangNewMoney}} RMB</b>
                                                <b v-else>{{item.xiaoyuanfengguangValue}}</b>
                                            </h4>
                                        </div>
                                    </a>
                                </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            xiaoyuanfengguangCollectionTypesList : [],
        // 级联表字典表数据
            xiaoyuanfengguangTypesList : [],
            shangxiaTypesList : [],
            xiaoyuanfengguangCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteXiaoyuanfengguangCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('xiaoyuanfengguangCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function xiaoyuanfengguangCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xiaoyuanfengguang_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.xiaoyuanfengguangCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function xiaoyuanfengguangTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xiaoyuanfengguang_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.xiaoyuanfengguangTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#xiaoyuanfengguangUuidNumber').val() != null && jquery('#xiaoyuanfengguangUuidNumber').val() != "" && jquery('#xiaoyuanfengguangUuidNumber').val() != "null" ) {
                param['xiaoyuanfengguangUuidNumber'] = jquery('#xiaoyuanfengguangUuidNumber').val();
            }
                     
            if (jquery('#xiaoyuanfengguangName').val() != null && jquery('#xiaoyuanfengguangName').val() != "" && jquery('#xiaoyuanfengguangName').val() != "null" ) {
                param['xiaoyuanfengguangName'] = jquery('#xiaoyuanfengguangName').val();
            }
                                         
            var xiaoyuanfengguangTypes = $("#xiaoyuanfengguangTypes").find("option:selected").val();
            if (xiaoyuanfengguangTypes != null && xiaoyuanfengguangTypes != "" && xiaoyuanfengguangTypes != "null") {
                            param['xiaoyuanfengguangTypes'] = xiaoyuanfengguangTypes;
                        }
                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['xiaoyuanfengguangCollectionTypes'] = 1;

            // 获取列表数据
            http.request('xiaoyuanfengguangCollection/page', 'get', param, function(res) {
